...
ax.set_title(f"Driven Drum Head\nf = {freq:.3f} Hz, Wavelengths per Radius = 5")
ax.axis('off')
...
def update(frame):
    ...
    for _ in range(8):
        ...
        if 2 < ix < NRes-2 and 2 < iy < NRes-2:
            grad_x = (energy_envelope[iy, ix+1] - energy_envelope[iy, ix-1]) / (2 * dx)
            grad_y = (energy_envelope[iy+1, ix] - energy_envelope[iy-1, ix]) / (2 * dx)
            
            # Slide away from high energy (antinodes) towards low energy (nodes)
            osc_x -= drift_speed * grad_x * dt
            osc_y -= drift_speed * grad_y * dt
    ...
    title = ax.set_title(f"Driven Drum Head\nf = {freq:.3f} Hz, X = {osc_x:.2f}, Y = {osc_y:.2f}")
    
    return im, osc_marker, title
